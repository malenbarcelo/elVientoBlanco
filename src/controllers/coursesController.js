...
const coursesController = {
    ...
    viewDocument: async(req,res) => {
        try{
            ...
            const issueDateMonth = months[issueDate.getMonth()]
            const issueDateDay = issueDate.getDate()
            const issueDateString = ' a los ' + issueDateDay + ' días del mes de ' + issueDateMonth + ' de ' + issueDateYear
            const issueDateString2 = await datesFunctions.dateToString(issueDate)
           ...
           if (validity != 0) {
            ...
            if (typeOfDocument == 'certificates') {
                return res.render('courses/certificates',{title:'Certificado',documentCode,documentTemplate,documentData,issueDateString,expirationDateString,studentImage})
            }else{
                return res.render('courses/credentials',{title:'Credencial',documentCode,documentTemplate,documentData,issueDateString2,expirationDateString,studentImage})
            }
        }catch(error){
            console.log(error)
            return res.send('Ha ocurrido un error')
